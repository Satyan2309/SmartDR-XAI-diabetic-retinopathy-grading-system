...
def login(req: LoginRequest):
    ...
    try:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            ...
            doctor = dict(row)
            return {"token": create_token(doctor["doctor_id"]), "doctor": doctor}
    finally:
        conn.close()
...
def register(req: RegisterRequest):
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute("SELECT COUNT(*) FROM doctors WHERE doctor_id=%s", (doc_id,))
            ...
            cur.execute(
                ...
                (
                    doc_id, req.name.strip(), req.email.strip(),
                    hash_password(req.password), req.hospital.strip(),
                    "Ophthalmology", req.phone.strip(), req.city.strip(),
        ...
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM doctors WHERE doctor_id=%s", (doc_id,))
            doctor = dict(cur.fetchone())
            return {"token": create_token(doc_id), "doctor": doctor}
...
def update_profile(req: UpdateProfileRequest, doctor=Depends(get_current_doctor)):
    conn = get_db_connection()
    try:
        ...
        with conn.cursor() as cur:
            for col, val in updates.items():
                cur.execute(
                    f"UPDATE doctors SET {col}=%s WHERE doctor_id=%s",
                    (val, doctor["doctor_id"])
                )
            conn.commit()
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("SELECT * FROM doctors WHERE doctor_id=%s", (doctor["doctor_id"],))
...
def change_password(req: ChangePasswordRequest, doctor=Depends(get_current_doctor)):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                (hash_password(req.new_password), doctor["doctor_id"])
            )
            conn.commit()
...
async def scan_image(
    ...
    return {
        ...
        "original_img" : _img_to_base64(image_array),
        "enhanced_img" : _img_to_base64(enhanced),
        "heatmap_img"  : _img_to_base64(heatmap),
    }
...
def save_record(
    ...
    if img_original or img_enhanced or img_heatmap:
        ...
        for name, b64 in [
            ...
            if b64:
                img_dir.joinpath(f"{name}.png").write_bytes(base64.b64decode(b64))
...
def download_pdf(record_id: str, doctor=Depends(get_current_doctor)):
...
def analytics(doctor=Depends(get_current_doctor)):
    ...
    grade_dist  = [0] * 5
    urgency_map = {"LOW": 0, "MEDIUM": 0, "HIGH": 0, "CRITICAL": 0}
    daily       = {}
    ...
    return {
        ...
        "daily_scans": [{"date": k, "count": v} for k, v in sorted(daily.items())],
        "high_risk"  : [r for r in records if r["grade"] >= 3],
    }
